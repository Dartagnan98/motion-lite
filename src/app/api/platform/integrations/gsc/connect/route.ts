// POST /api/platform/integrations/gsc/connect
//
// Body: { level: 'agency'|'account'|'domain',
//         workspaceId?: number,
//         agencyId?: number, accountId?: number, domainId?: number }
//
// If workspaceId is provided, the caller must be a member (FORBIDDEN otherwise).
// If omitted, falls back to the user's primary workspace.
// If accountId is omitted at level=account, falls back to the first active
// account in the tenant. Same for agencyId at level=agency.
//
// Resolves (or creates) the platform tenant for the workspace, then calls
// gscAdapter.connect() which builds the Google OAuth URL with a signed state.
// Returns { redirectUrl } — the caller redirects the browser there.
...
export async function POST(req: NextRequest) {
  ...
  // domain level still requires explicit FK — domain selection has no sane default.
  if (level === 'domain' && !domainId) {
    return NextResponse.json({ error: 'domainId required when level=domain' }, { status: 400 })
  }

  try {
    // Resolve workspace: explicit body wins (membership-checked), else primary.
    ...
    if (level === 'account' && !accountId) {
      ...
      if (!accountId) {
        return NextResponse.json({ error: 'No active client selected. Create a client first.' }, { status: 400 })
      }
    }
    if (level === 'agency' && !agencyId) {
      ...
      if (!row) {
        return NextResponse.json({ error: 'No agencies seeded for this tenant. Run scripts/seed-glenvalley.mjs.' }, { status: 400 })
      }
      agencyId = row.id
    }
    ...
    const result = await gscAdapter.connect({
      tenantId: tenant.id,
      level,
      agencyId,
      accountId,
    ...
    console.error('[platform/gsc/connect]', message)
