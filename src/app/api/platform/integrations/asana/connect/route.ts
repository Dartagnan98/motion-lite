// POST /api/platform/integrations/asana/connect
//
// Mirrors /api/platform/integrations/ga4/connect exactly. Body shape is
...
// The caller (connect page) navigates the browser to Asana's consent screen.
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Fill in account FK — prefer active account cookie, then first row.
    ...
    const result = await asanaAdapter.connect({
      tenantId: tenant.id,
      level,
      agencyId,
      accountId,
    ...
    console.error('[platform/asana/connect]', message)
