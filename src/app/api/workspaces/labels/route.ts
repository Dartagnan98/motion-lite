...
import { getWorkspaceLabels, createWorkspaceLabel, updateWorkspaceLabel, deleteWorkspaceLabel, getDb } from '@/lib/db'
...
export async function GET(request: NextRequest) {
  ...
  return NextResponse.json(getWorkspaceLabels(workspaceId))
}
...
export async function POST(request: NextRequest) {
  ...
  const label = createWorkspaceLabel(wsId, name, color || '#7a6b55')
...
export async function PATCH(request: NextRequest) {
  ...
  const updated = updateWorkspaceLabel(id, data)
