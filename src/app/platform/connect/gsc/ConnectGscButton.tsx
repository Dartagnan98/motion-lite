...
// "Connect a different Google account" — starts a fresh OAuth dance. The common
// path (reuse an existing agency identity) is plain links rendered server-side
// in page.tsx; this is only for adding a new Google identity.

import { useState } from 'react'

export function ConnectGscButton({ label = 'Connect with Google' }: { label?: string }) {
  ...
  async function handleConnect() {
    ...
    setError(null)
    try {
      const res = await fetch('/api/platform/integrations/gsc/connect', {
        method: 'POST',
